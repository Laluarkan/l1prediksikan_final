...
export default function HistoryParlaysPage() {
  ...
  useEffect(() => {
    ...
    const params: Record<string, any> = {
      ordering: '-date',
      is_historical: true,
      // Ambil seluruh tiket parlay historis yang cocok filter dalam satu request
      // (bukan cuma 20 data pertama dari default pagination backend).
      page_size: 1000,
    };
        ...
        <div className="bg-slate-800 border border-slate-700 p-8 md:p-10 rounded-2xl max-w-md w-full text-center shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-8">
              Data historis performa tiket Parlay beserta kalkulasi ROI-nya hanya tersedia untuk pengguna terdaftar.
            </p>
            <button
  ...
  const displayedTickets = sortedTickets.filter(ticket => {
    if (filterStatus === 'WON' && ticket.is_won !== true) return false;
    if (filterStatus === 'LOST' && ticket.is_won !== false) return false;
    return true;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto pt-6 pb-12 px-4">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white tracking-wide">Historical Parlays</h1>
        <p className="text-slate-400 text-sm mt-1">Evaluasi riwayat tiket kombinasi value bet dan hasil akhirnya berdasarkan probabilitas matematis.</p>
      </div>

      <div className="flex flex-col lg:flex-row gap-6">
        <div className="w-full lg:w-72 flex-shrink-0 space-y-4">
          <div className="bg-slate-800 border border-slate-700 p-5 rounded-xl shadow-sm">
            <h2 className="text-sm font-bold text-white mb-4">Pengaturan Evaluasi</h2>
            <div>
              <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Nilai Simulasi (Rp)</label>
              ...
              <p className="text-[10px] text-slate-500 mt-2 leading-relaxed">Nilai ini digunakan untuk menghitung ulang performa Return on Investment (ROI) historis.</p>
            </div>
          </div>

          <div className="bg-slate-800 border border-slate-700 p-5 rounded-xl shadow-sm">
            <h2 className="text-sm font-bold text-white mb-4">Filter Historis</h2>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Status Tiket</label>
                <select
                  value={filterStatus}
        ...
        <div className="flex-1 space-y-4">
          {loading ? (
            <div className="text-center py-12 text-sm text-slate-400 bg-slate-800/50 border border-slate-700 rounded-xl">Memuat data riwayat parlay...</div>
          ) : currentTickets.length === 0 ? (
            <div className="text-center py-12 text-sm text-slate-400 bg-slate-800 border border-slate-700 rounded-xl">Tidak ada tiket parlay historis yang ditemukan.</div>
          ) : (
            <>
              {totalPages > 1 && (
              ...
              {currentTickets.map((ticket) => {
                const potentialPayout = ticketStake * ticket.total_odds;
                const actualPayout = ticket.is_won ? potentialPayout : 0;
                  ...
                  <div key={ticket.id} className={`bg-slate-800 border ${isExpanded ? 'border-blue-500/50' : 'border-slate-700'} rounded-xl overflow-hidden shadow-sm transition-colors`}>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-xs text-white font-mono font-bold">{ticket.ticket_id || `PRL-H-${ticket.id}`}</span>
                      ...
                      <div>
                        {ticket.is_won === true && <span className="bg-emerald-900/40 border border-emerald-700 text-emerald-400 text-[10px] px-2.5 py-1 rounded font-semibold tracking-wide">MENANG</span>}
                        {ticket.is_won === false && <span className="bg-rose-900/40 border border-rose-700 text-rose-400 text-[10px] px-2.5 py-1 rounded font-semibold tracking-wide">KALAH</span>}
                        {ticket.is_won === null && <span className="bg-slate-800 border border-slate-600 text-slate-400 text-[10px] px-2.5 py-1 rounded font-semibold tracking-wide">PENDING</span>}
                      </div>
                    </div>
                    ...
                    <div className="p-5 grid grid-cols-1 md:grid-cols-[1.5fr_1fr] gap-6 items-center">
                      ...
                      <div className="space-y-3 pl-0 md:pl-6 md:border-l border-slate-700/50">
                        ...
                        <div className="flex justify-between items-center">
                          <span className="text-xs text-slate-400">Actual Payout:</span>
                          <span className={`text-sm font-bold ${ticket.is_won ? 'text-emerald-400' : 'text-slate-500'}`}>
                            {ticket.is_won ? '+' : ''}{formatCurrency(actualPayout)}
                          </span>
                        </div>
                      </div>
                    </div>
